    ...
    jQuery.ajax({
      ...
      success: function(data) {
        ...
        for (let i = 0; i < data["totalCart"]; i++) {
          productsTable += '' +
            '<tr id="' + data["cart"][i].sku + '" class="table-row" data-sku="' + data["cart"][i].sku + '" data-name="cart">' +
              '<td class="column-1">' +
                '<div class="cart-img-product b-rad-4 o-f-hidden" data-sku="' + data["cart"][i].sku + '">' +
                  '<img src="' + URLROOT + '/' + data["cart"][i].imgPath + '" alt="IMG-PRODUCT">' +
                '</div>' +
              '</td>' +
              '<td class="column-2"><a href="' + URLROOT + '/products/product/' + data["cart"][i].sku + '">' + data["cart"][i].productName + '</a></td>' +
              '<td class="column-3">$' + data["cart"][i].price + '</td>' +
              '<td class="column-4">' +
                '<div class="flex-w bo5 of-hidden w-size17">' +
                  ...
                  '<input class="size8 m-text18 t-center num-product" type="number" name="num-product1" value="' + data["cart"][i].quantity + '">' +
                ...
                '</div>' +
              '</td>' +
              '<td class="column-5">$' + myToFixed(parseFloat(data["cart"][i].quantity) * parseFloat(data["cart"][i].price), 3) + '</td>' +
            '</tr>';
        }
